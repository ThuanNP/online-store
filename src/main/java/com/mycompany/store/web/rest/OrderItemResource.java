...
public class OrderItemResource {
     ...
     *         the orderItem has already an ID.
    ...
    public ResponseEntity<OrderItem> createOrderItem(@Valid @RequestBody OrderItem orderItem)
        ...
        return ResponseEntity
                .created(new URI("/api/order-items/" + result.getId())).headers(HeaderUtil
     ...
     *         the orderItem is not valid, or with status
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the orderItem, or with status {@code 404 (Not Found)}.
     */
