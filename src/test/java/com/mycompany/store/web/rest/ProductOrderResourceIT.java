...
public class ProductOrderResourceIT {
    ...
    public static ProductOrder createEntity(EntityManager em) {
        ProductOrder productOrder = new ProductOrder().placeDate(DEFAULT_PLACE_DATE).status(DEFAULT_STATUS)
                .code(DEFAULT_CODE);
        // Add required entity
        Customer customer;
    ...
    public static ProductOrder createUpdatedEntity(EntityManager em) {
        ProductOrder productOrder = new ProductOrder().placeDate(UPDATED_PLACE_DATE).status(UPDATED_STATUS)
                .code(UPDATED_CODE);
        // Add required entity
        Customer customer;
        if (TestUtil.findAll(em, Customer.class).isEmpty()) {
    ...
    public void createProductOrder() throws Exception {
        ...
        restProductOrderMockMvc.perform(post("/api/product-orders").contentType(MediaType.APPLICATION_JSON)
                .content(TestUtil.convertObjectToJsonBytes(productOrder))).andExpect(status().isCreated());
    ...
    public void createProductOrderWithExistingId() throws Exception {
        ...
        restProductOrderMockMvc.perform(post("/api/product-orders").contentType(MediaType.APPLICATION_JSON)
                .content(TestUtil.convertObjectToJsonBytes(productOrder))).andExpect(status().isBadRequest());
    ...
    public void getAllProductOrders() throws Exception {
        ...
        restProductOrderMockMvc.perform(get("/api/product-orders?sort=id,desc")).andExpect(status().isOk())
    ...
    public void updateProductOrder() throws Exception {
        ...
        // Disconnect from session so that the updates on updatedProductOrder are not
        ...
        restProductOrderMockMvc.perform(put("/api/product-orders").contentType(MediaType.APPLICATION_JSON)
                .content(TestUtil.convertObjectToJsonBytes(updatedProductOrder))).andExpect(status().isOk());
